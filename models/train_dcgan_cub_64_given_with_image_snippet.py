...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.noise_dim = 100
        self.embed_dim = 1024
        self.image_snippet_size = 5
        self.projected_embed_dim = 128
        self.latent_dim = self.noise_dim + self.projected_embed_dim + self.image_snippet_size**2
        self.ngf = 64
        ...
        self.image_snippet_projection = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=6, kernel_size=(2, 2)),
            nn.BatchNorm2d(6),
            nn.Flatten(),
            nn.Linear(96, 25)
        )
    ...
    def forward(self, embed_vector, z, image_snippet):
        image_snippet_projection = self.image_snippet_projection(image_snippet).unsqueeze(2).unsqueeze(3)
        projected_embed = self.projection(embed_vector).unsqueeze(2).unsqueeze(3)
        latent_vector = torch.cat([projected_embed, z, image_snippet_projection], 1)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.netD_2 = nn.Sequential(
            ...
            nn.Conv2d(self.ndf * 8 + self.projected_embed_dim + self.image_snippet_dim, 1, 4, 1, 0, bias=False),
            nn.Sigmoid()
        )

    def forward(self, inp, embed, image_snippet):

        image_snippet = self.image_snippet_projection(image_snippet).reshape((image_snippet.shape[0], 6, 4, 4))
        x_intermediate = self.netD_1(inp)
        x = self.projector(x_intermediate, embed)
        x = torch.cat([x, image_snippet], 1)
        x = self.netD_2(x)
...
def main():
    ...
    netG = Generator()
    print("Finished setting up Generator", flush=True)

    # Create the disciminator
    netD = Discriminator()
    print("Finished setting up Discriminator", flush=True)
    ...
    fixed_images_snippet = Variable(fixed_images[:, :, :5, :5].float()).cuda()
    ...
    for epoch in range(params['num_epochs']):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            right_image_snippet = Variable(right_images[:, :, :5, :5].float()).cuda()
            ...
            netD.zero_grad()
            outputs, activation_real = netD(right_images, right_embed, right_image_snippet)
            real_loss = criterion(outputs, smoothed_real_labels)
            real_score = outputs

            outputs, _ = netD(wrong_images, right_embed, right_image_snippet)
            wrong_loss = criterion(outputs, fake_labels)
            wrong_score = outputs

            noise = Variable(torch.randn(right_images.size(0), 100)).cuda()
            noise = noise.view(noise.size(0), 100, 1, 1)
            fake_images = netG(right_embed, noise, right_image_snippet)
            outputs, _ = netD(fake_images, right_embed, right_image_snippet)
            ...
            noise = noise.view(noise.size(0), 100, 1, 1)
            fake_images = netG(right_embed, noise, right_image_snippet)
            outputs, activation_fake = netD(fake_images, right_embed, right_image_snippet)
            _, activation_real = netD(right_images, right_embed, right_image_snippet)
            ...
            if i % 50 == 0:
                ...
                results = {
                    "epoch": epoch,
                    "d_loss": str(d_loss.data.cpu().mean()),
                    "g_loss": str(g_loss.data.cpu().mean()),
                    "d_x": str(real_score.data.cpu().mean()),
                    "d_g_x": str(fake_score.data.cpu().mean())
                }
            ...
            if (i % 500 == 0) or ((epoch == params['num_epochs'] - 1) and (i == len(dataloader) - 1)):
                ...
                with torch.no_grad():
                    fake = netG(fixed_captions, fixed_noise, fixed_images_snippet)
        ...
        if rank == 0:
            ...
            bucket_manager.save_object("text-captions/{}/results/epoch_{}.json".format(args.model_name, epoch), results, json)
