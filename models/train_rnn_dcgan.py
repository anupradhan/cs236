...
from models.utils import weights_init, download_datasets, set_seed, word_index, vectorize_caption
...
class ReverseDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        captions = vectorize_caption(self.word_to_index, target[i % 5], copies=2)

        captions, cap_lens = self.get_caption(captions)
        j = 1
        while cap_lens == 0:
            print(target[i % 5], captions, cap_lens)
            captions = vectorize_caption(self.word_to_index, target[((i % 5)+j) % 5], copies=2, debug=True)
            captions, cap_lens = self.get_caption(captions)
            j += 1

        return img, captions, cap_lens
...
def main():
    ...
    download_datasets()
    bucket_manager.download_object("text-captions/captions.pickle", "captions.pickle")
    ...
    img, captions, cap_lens = dataset[3]
    ...
    fixed_images, fixed_captions, fixed_cap_lens = iter(dataloader).next()
    ...
    for epoch in range(params['num_epochs']):
        ...
        def process_iteration():
            ...
            word_embeddings, target = text_encoder(captions, cap_lens, hidden)
