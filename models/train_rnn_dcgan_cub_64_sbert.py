...
class ReverseDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, word_to_index, index_to_word, caption_embeddings):
    ...
    def __getitem__(self, i):
        ...
        target_embedding = self.caption_embeddings[target[i % 10]]
        fake_target_embedding = self.caption_embeddings[fake_target[i % 10]]

        captions, cap_lens = self.rotate_caption(target, i)
        fake_captions, fake_cap_lens = self.rotate_caption(fake_target, i)

        return real_img, target_embedding, cap_lens, fake_img, fake_target_embedding, fake_cap_lens
...
def main():
    ...
    dataset = ReverseDataset(dataset, word_to_index, index_to_word, caption_embeddings)
